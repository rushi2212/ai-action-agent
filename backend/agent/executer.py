    ...
    try:
        with sync_playwright() as p:
            ...
            for i, step in enumerate(plan["steps"], 1):
                try:
                    action = step["action"]
                    log(f"Step {i}/{len(plan['steps'])}: {action}")
                    ...
                    if action == "goto":
                        ...
                        field_identifier = step.get(
                            "field_identifier", "").lower()
                                    ...
                                    for select_elem in select_elements:
                                        if select_elem.is_visible():
                                            ...
                                            try:
                                                ...
                                                log(
                                                    f"  ✓ Selected '{option_text}' from dropdown")
                                                found = True
                                                break
                                            except:
                                                try:
                    ...
                    elif action == "find_and_click_product":
                    ...
                    elif action == "find_and_click_button":
                            ...
                            try:
                                if page.locator(sel).count() > 0:
                                    ...
                                    if text and text.strip():
                                        ...
                                        log(
                                            f"  ✓ Extracted {label}: {text.strip()} using {sel}")
                                        found = True
                                        break
                            except:
                                continue
                    ...
                    elif action == "extract_smart_results":
                        ...
                        try:
                            ...
                            log(
                                f"  🔍 Checking {selector} for {label}: found {element_count} element(s)")
                            ...
                            if element_count > 0:
                                ...
                                log(
                                    f"  ⚠️ Element not found: {selector} for {label}")
                        except Exception as extract_error:
                            log(f"  ⚠️ Failed to extract {label}: {str(extract_error)}")

                except Exception as e:
                    log(f"  ❌ Error in step {i}: {str(e)}")
                    if step["action"] in critical_actions:
                        log("  ⛔ Critical step failed; stopping execution.")
                        raise
                    log("  → Continuing with next step...")
        ...
        if browser is not None:
            try:
                ...
                log(f"⚠️ Browser close failed: {str(close_error)}")
